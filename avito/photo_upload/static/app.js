(function () {
  ...
  async function fetchShotGuide(index) {
    const response = await fetch(`api/shot-guide?index=${encodeURIComponent(index)}`);
    if (!response.ok) {
  ...
  async function updateNextShotHint() {
    ...
    try {
      ...
      nextShotHint.innerHTML = `Следующий кадр: <strong>фото ${meta.index} — ${guide.title}</strong>`;
    } catch (_error) {
  ...
  async function openCameraModal() {
    ...
    try {
      meta = await resolveNextMeta(article);
      guide = await fetchShotGuide(meta.index);
    } catch (error) {
      showToast(String(error.message || error));
      return;
    }

    activeShotIndex = meta.index;
    cameraTitle.textContent = `Фото ${meta.index}`;
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      showToast("Камера в браузере недоступна — используйте кнопку ниже");
      return;
    }
    ...
    try {
      ...
      showToast("Не удалось открыть камеру — используйте системную камеру");
    }
  }

  async function captureFromCamera() {
    if (!cameraVideo || !cameraStream) {
      showToast("Камера не запущена — используйте системную камеру");
      return;
    }
